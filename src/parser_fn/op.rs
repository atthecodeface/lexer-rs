    ...
    M: Fn(T) -> R,
{
    ...
    move |input| match f(input)? {
        Matched(input, t) => Ok(Matched(input, m(t))),
...
//a Fold
//fp fold
pub fn fold<P, I: ParserInputStream<P>, R, F, G, H, T>(
    max: usize,
    fold: H,
    f: F,
    g: G,
    ...
    F: Fn(I) -> ParseFnResult<P, R>,
{
    use ParseResult::*;
    move |input| match f(input)? {
        Matched(input, r) => Ok(Matched(input, Some(r))),
        Mismatched => Ok(Matched(input, None)),
    }
}
    ...
    R: Copy,
{
    use ParseResult::*;
    move |input| match f(input)? {
        Matched(_, _) => Ok(Mismatched),
        Mismatched => Ok(Matched(input, r)),
    }
}
    ...
    E: Fn(I) -> ParseFnResult<P, R>,
{
    use ParseResult::*;
    move |input| match f(input)? {
        Matched(input, r) => Ok(Matched(input, r)),
        Mismatched => e(input),
    }
}

//fp unnwrap_or_else
pub fn unwrap_or_else<P, I: ParserInputStream<P>, R, F, E, G>(
    f: F,
    e: G,
) -> impl Fn(I) -> Result<R, E>
where
