...
fn parse_value_fn<L:LexerOfChar<State = TextPos, Token = LexToken, Error = LexError>>(stream: &L, state: TextPos, ch: char) -> LexerParseResult<TextPos, LexToken, LexError> {
...
type BoxDynLexTokenLexFn<'a> =  Box<dyn for<'call, 'stream> Fn(&'call TextStream<'stream>, TextPos, char) ->
                                       LexerParseResult<TextPos, LexToken, LexError> + 'a>;
use std::env;
#[derive(Default)]
struct ParserVec<'a> {
...
impl<'a> ParserVec<'a> {
    ...
    pub fn add_parser<F>(&mut self, f: F)
        where F: Fn(&TextStream, TextPos, char) -> LexerParseResult<TextPos, LexToken, LexError> + 'static
    {
